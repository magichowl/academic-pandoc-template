...
function Sync-Macros-LuaToTex {
    Write-Host "[Syncing macros from Lua to TeX...]" -ForegroundColor Yellow
    ...
    $texContent = @()
    $texContent += "% Macro definition file"
    $texContent += "% This file is auto-generated by build.ps1, do not edit manually"
    $texContent += "% All macros are defined in expand-macros.lua"
    $texContent += ""
    ...
    foreach ($line in $lines) {
        ...
        $parts = $lineEscaped -split "\s*=\s*", 2
        if ($parts.Count -ne 2) {
            continue
        }
        
        $keyPart = $parts[0].Trim()
        $valuePart = $parts[1].TrimEnd(",").Trim()
        
        $keyMatch = [regex]::Match($keyPart, '\[\s*"\\(\w+)"\s*\]')
        if ($keyMatch.Success) {
            $name = $keyMatch.Groups[1].Value
            
            if ($valuePart.StartsWith("{")) {
                $valueMatch = [regex]::Match($valuePart, '^\{\s*"([^"]+)"\s*,\s*(\d+)\s*\}$')
                if ($valueMatch.Success) {
                    $body = $valueMatch.Groups[1].Value.Replace('%', '#')
                    $numParams = $valueMatch.Groups[2].Value
                    $texContent += "\newcommand{\${name}}[$numParams]{$body}"
                }
            } else {
                $valueMatch = [regex]::Match($valuePart, '^"([^"]+)"$')
                if ($valueMatch.Success) {
                    $body = $valueMatch.Groups[1].Value
                    $texContent += "\newcommand{\${name}}{$body}"
                }
            }
        }
    ...
    Write-Host "  Macros synced to $texFile" -ForegroundColor Green
}
